...
kbash.e = function(string) {
  var string = (string || 'ERROR:')
  return '<span class="red">' + string + '</span>' 
}
kbash.s = function(string) {
  var string = (string || 'SUCCESS:')
...
kbash.math = function(args, flags, opts, props) {
  if (props.empty) { console.kbash('Please provide an equation to calculate') }
  for ( var i =0; i < args.length; i++ ) {
    try {
      if (args[i].match(/^[-*/+0-9]+$/)) { 
        ...
        console.kbash(kbash.e() + ' Insecure string detected') 
        console.kbash('To prevent a script attack, this input will not be calculated.')
      }
    }
    catch(e) {
      console.kbash(kbash.e() + ' Could not compute.')
      console.kbash('(' + e + ')');
    }
  }
}
kbash.edit = function() {
  ...
  console.kbash(kbash.s('EDITING MODE') + ': You may now edit this website.')
}
