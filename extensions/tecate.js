kbash.check = function(args, flags, opts, props) {
        Tecate = new Object();
        ...
        Tecate.appendError = function(error) {
            console.kbash(error.errorString + Tecate.escapeHTML(error.error) + ' on line ' + error.line)
        ...
        Tecate.showErrors = function(errorsList) {
            ...
            for (var i = 0; i < errorsList.length; i++) {
                if (flags.have('c')) { msg++ }
                else { Tecate.appendError(errorsList[i]); }
            }
            if (flags.have('c')) { console.kbash(msg + ' error(s) found')}
        };
